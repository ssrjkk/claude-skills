---
name: nomad-automation
description: "Automation for Nomad: automation and scripting"
category: devops
tags: [nomad-automation, devops, russian]
models: [sonnet, opus]
version: "1.0"
language: ru
original: nomad-automation
---

# Nomad Automation

> Automation for Nomad: automation and scripting

## Быстрый старт
Этот навык на русском языке. Оригинал: nomad-automation.

## Когда использовать
- Работа с DevOps
- Выполнение задач, связанных с Nomad Automation

## Инструкции
1. Ознакомьтесь с описанием
2. Изучите английскую версию
3. Примените знания на практике

## Ресурсы
- devops/nomad-automation/SKILL.md
- Категория: DevOps

## Валидация
- Прочитайте английскую версию
- Выполните шаги
- Проверьте результат
