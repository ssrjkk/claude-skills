---
name: kustomize-fundamentals
description: Fundamentals for Kustomize: core concepts
category: devops
tags: [kustomize-fundamentals, devops, russian]
models: [sonnet, opus]
version: "1.0"
language: ru
original: kustomize-fundamentals
---

# Kustomize Fundamentals

> Fundamentals for Kustomize: core concepts

## Быстрый старт
Этот навык доступен на русском языке. Оригинал: `kustomize-fundamentals`.

## Когда использовать
- Работа с DevOps
- Выполнение задач, связанных с Kustomize Fundamentals

## Инструкции
1. Изучите английскую версию навыка для полных инструкций
2. Примените полученные знания на практике
3. Проверьте результат выполнения

## Ресурсы
- Оригинал: devops/kustomize-fundamentals/SKILL.md
- Категория: DevOps

## Валидация
- Выполните шаги из английской версии
- Убедитесь в правильности результата
