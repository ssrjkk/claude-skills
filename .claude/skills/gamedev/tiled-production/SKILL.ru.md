---
name: tiled-production
description: "Production for Tiled: production deployment"
category: gamedev
tags: [tiled-production, gamedev, russian]
models: [sonnet, opus]
version: "1.0"
language: ru
original: tiled-production
---

# Tiled Production

> Production for Tiled: production deployment

## Быстрый старт
Этот навык на русском языке. Оригинал: tiled-production.

## Когда использовать
- Работа с Геймдев
- Выполнение задач, связанных с Tiled Production

## Инструкции
1. Ознакомьтесь с описанием
2. Изучите английскую версию
3. Примените знания на практике

## Ресурсы
- gamedev/tiled-production/SKILL.md
- Категория: Геймдев

## Валидация
- Прочитайте английскую версию
- Выполните шаги
- Проверьте результат
