---
name: aesprite-production
description: Production for Aesprite: production deployment
category: gamedev
tags: [aesprite-production, gamedev, russian]
models: [sonnet, opus]
version: "1.0"
language: ru
original: aesprite-production
---

# Aesprite Production

> Production for Aesprite: production deployment

## Быстрый старт
Этот навык на русском языке. Оригинал: aesprite-production.

## Когда использовать
- Работа с Геймдев
- Выполнение задач, связанных с Aesprite Production

## Инструкции
1. Ознакомьтесь с описанием
2. Изучите английскую версию
3. Примените знания на практике

## Ресурсы
- gamedev/aesprite-production/SKILL.md
- Категория: Геймдев

## Валидация
- Прочитайте английскую версию
- Выполните шаги
- Проверьте результат
