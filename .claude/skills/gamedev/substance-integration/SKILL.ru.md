---
name: substance-integration
description: Integration for Substance: integration with other tools
category: gamedev
tags: [substance-integration, gamedev, russian]
models: [sonnet, opus]
version: "1.0"
language: ru
original: substance-integration
---

# Substance Integration

> Integration for Substance: integration with other tools

## Быстрый старт
Этот навык на русском языке. Оригинал: substance-integration.

## Когда использовать
- Работа с Геймдев
- Выполнение задач, связанных с Substance Integration

## Инструкции
1. Ознакомьтесь с описанием
2. Изучите английскую версию
3. Примените знания на практике

## Ресурсы
- gamedev/substance-integration/SKILL.md
- Категория: Геймдев

## Валидация
- Прочитайте английскую версию
- Выполните шаги
- Проверьте результат
