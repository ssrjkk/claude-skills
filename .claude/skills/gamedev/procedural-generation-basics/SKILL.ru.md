---
name: procedural-generation-basics
description: Basics with Procedural Generation. fundamentals.
category: gamedev
tags: [procedural-generation-basics, gamedev, russian]
models: [sonnet, opus]
version: "1.0"
language: ru
original: procedural-generation-basics
---

# Procedural Generation Basics

> Basics with Procedural Generation. fundamentals.

## Быстрый старт
Этот навык на русском языке. Оригинал: `procedural-generation-basics`.

## Когда использовать
- Работа с Геймдев
- Выполнение задач, связанных с Procedural Generation Basics
- Профессиональное развитие

## Инструкции
1. Ознакомьтесь с описанием навыка
2. Изучите английскую версию для полных инструкций
3. Примените полученные знания на практике

## Ресурсы
- Оригинальный навык: `gamedev/procedural-generation-basics/SKILL.md`
- Категория: Геймдев
- Язык: Русский

## Валидация
- Прочитайте английскую версию для проверки
- Выполните описанные шаги
- Убедитесь в правильности результата
