---
name: angelist-interviewing
description: "Interviewing with AngelList/Wellfound. interviews."
category: hr
tags: [angelist-interviewing, hr, russian]
models: [sonnet, opus]
version: "1.0"
language: ru
original: angelist-interviewing
---

# Angelist Interviewing

> Interviewing with AngelList/Wellfound. interviews.

## Быстрый старт
Этот навык на русском языке. Оригинал: `angelist-interviewing`.

## Когда использовать
- Работа с HR
- Выполнение задач, связанных с Angelist Interviewing
- Профессиональное развитие

## Инструкции
1. Ознакомьтесь с описанием навыка
2. Изучите английскую версию для полных инструкций
3. Примените полученные знания на практике

## Ресурсы
- Оригинальный навык: `hr/angelist-interviewing/SKILL.md`
- Категория: HR
- Язык: Русский

## Валидация
- Прочитайте английскую версию для проверки
- Выполните описанные шаги
- Убедитесь в правильности результата
