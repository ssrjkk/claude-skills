---
name: linkedin-recruiter-analytics
description: "Analytics with LinkedIn Recruiter. analytics."
category: hr
tags: [linkedin-recruiter-analytics, hr, russian]
models: [sonnet, opus]
version: "1.0"
language: ru
original: linkedin-recruiter-analytics
---

# Linkedin Recruiter Analytics

> Analytics with LinkedIn Recruiter. analytics.

## Быстрый старт
Этот навык на русском языке. Оригинал: `linkedin-recruiter-analytics`.

## Когда использовать
- Работа с HR
- Выполнение задач, связанных с Linkedin Recruiter Analytics
- Профессиональное развитие

## Инструкции
1. Ознакомьтесь с описанием навыка
2. Изучите английскую версию для полных инструкций
3. Примените полученные знания на практике

## Ресурсы
- Оригинальный навык: `hr/linkedin-recruiter-analytics/SKILL.md`
- Категория: HR
- Язык: Русский

## Валидация
- Прочитайте английскую версию для проверки
- Выполните описанные шаги
- Убедитесь в правильности результата
