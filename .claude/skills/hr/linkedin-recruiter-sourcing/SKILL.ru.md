---
name: linkedin-recruiter-sourcing
description: "Sourcing with LinkedIn Recruiter. candidate sourcing."
category: hr
tags: [linkedin-recruiter-sourcing, hr, russian]
models: [sonnet, opus]
version: "1.0"
language: ru
original: linkedin-recruiter-sourcing
---

# Linkedin Recruiter Sourcing

> Sourcing with LinkedIn Recruiter. candidate sourcing.

## Быстрый старт
Этот навык на русском языке. Оригинал: `linkedin-recruiter-sourcing`.

## Когда использовать
- Работа с HR
- Выполнение задач, связанных с Linkedin Recruiter Sourcing
- Профессиональное развитие

## Инструкции
1. Ознакомьтесь с описанием навыка
2. Изучите английскую версию для полных инструкций
3. Примените полученные знания на практике

## Ресурсы
- Оригинальный навык: `hr/linkedin-recruiter-sourcing/SKILL.md`
- Категория: HR
- Язык: Русский

## Валидация
- Прочитайте английскую версию для проверки
- Выполните описанные шаги
- Убедитесь в правильности результата
