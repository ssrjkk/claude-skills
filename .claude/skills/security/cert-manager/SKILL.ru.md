---
name: cert-manager
description: Manages TLS certificates in Kubernetes with cert-manager, Let's Encrypt, and auto-renewal.
category: security
tags: [cert-manager, security, russian]
models: [sonnet, opus]
version: "1.0"
language: ru
original: cert-manager
---

# Cert Manager

> Manages TLS certificates in Kubernetes with cert-manager, Let's Encrypt, and auto-renewal.

## Быстрый старт
Этот навык на русском языке. Оригинал: `cert-manager`.

## Когда использовать
- Работа с Безопасность
- Выполнение задач, связанных с Cert Manager
- Профессиональное развитие

## Инструкции
1. Ознакомьтесь с описанием навыка
2. Изучите английскую версию для полных инструкций
3. Примените полученные знания на практике

## Ресурсы
- Оригинальный навык: `security/cert-manager/SKILL.md`
- Категория: Безопасность
- Язык: Русский

## Валидация
- Прочитайте английскую версию для проверки
- Выполните описанные шаги
- Убедитесь в правильности результата
