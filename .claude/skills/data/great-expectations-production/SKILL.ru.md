---
name: great-expectations-production
description: Production for Great-Expectations: production deployment
category: data
tags: [great-expectations-production, data, russian]
models: [sonnet, opus]
version: "1.0"
language: ru
original: great-expectations-production
---

# Great Expectations Production

> Production for Great-Expectations: production deployment

## Быстрый старт
Этот навык на русском языке. Оригинал: great-expectations-production.

## Когда использовать
- Работа с Data Science
- Выполнение задач, связанных с Great Expectations Production

## Инструкции
1. Ознакомьтесь с описанием
2. Изучите английскую версию
3. Примените знания на практике

## Ресурсы
- data/great-expectations-production/SKILL.md
- Категория: Data Science

## Валидация
- Прочитайте английскую версию
- Выполните шаги
- Проверьте результат
