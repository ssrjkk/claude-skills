---
name: create-comprehensive-project-charter
description: "Develop a formal project charter that defines scope, objectives, and authority for a new project"
category: product
tags: [create-comprehensive-project-charter, product, russian]
models: [sonnet, opus]
version: "1.0"
language: ru
original: create-comprehensive-project-charter
---

# Create Comprehensive Project Charter

> Develop a formal project charter that defines scope, objectives, and authority for a new project

## Быстрый старт
Этот навык на русском языке. Оригинал: `create-comprehensive-project-charter`.

## Когда использовать
- Работа с Продукт-менеджмент
- Выполнение задач, связанных с Create Comprehensive Project Charter
- Профессиональное развитие

## Инструкции
1. Ознакомьтесь с описанием навыка
2. Изучите английскую версию для полных инструкций
3. Примените полученные знания на практике

## Ресурсы
- Оригинальный навык: `product/create-comprehensive-project-charter/SKILL.md`
- Категория: Продукт-менеджмент
- Язык: Русский

## Валидация
- Прочитайте английскую версию для проверки
- Выполните описанные шаги
- Убедитесь в правильности результата
