---
name: graphql-production
description: "Production for Graphql: production deployment"
category: backend
tags: [graphql-production, backend, russian]
models: [sonnet, opus]
version: "1.0"
language: ru
original: graphql-production
---

# Graphql Production

> Production for Graphql: production deployment

## Быстрый старт
Этот навык на русском языке. Оригинал: graphql-production.

## Когда использовать
- Работа с Бэкенд-разработка
- Выполнение задач, связанных с Graphql Production

## Инструкции
1. Ознакомьтесь с описанием
2. Изучите английскую версию
3. Примените знания на практике

## Ресурсы
- backend/graphql-production/SKILL.md
- Категория: Бэкенд-разработка

## Валидация
- Прочитайте английскую версию
- Выполните шаги
- Проверьте результат
