---
name: victoria-metrics-querying
description: "Querying with VictoriaMetrics. time-series queries."
category: database
tags: [victoria-metrics-querying, database, russian]
models: [sonnet, opus]
version: "1.0"
language: ru
original: victoria-metrics-querying
---

# Victoria Metrics Querying

> Querying with VictoriaMetrics. time-series queries.

## Быстрый старт
Этот навык на русском языке. Оригинал: `victoria-metrics-querying`.

## Когда использовать
- Работа с Базы данных
- Выполнение задач, связанных с Victoria Metrics Querying
- Профессиональное развитие

## Инструкции
1. Ознакомьтесь с описанием навыка
2. Изучите английскую версию для полных инструкций
3. Примените полученные знания на практике

## Ресурсы
- Оригинальный навык: `database/victoria-metrics-querying/SKILL.md`
- Категория: Базы данных
- Язык: Русский

## Валидация
- Прочитайте английскую версию для проверки
- Выполните описанные шаги
- Убедитесь в правильности результата
