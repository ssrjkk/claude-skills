---
name: backtesting-strategy
description: "Strategy with Backtesting. strategy dev."
category: finance
tags: [backtesting-strategy, finance, russian]
models: [sonnet, opus]
version: "1.0"
language: ru
original: backtesting-strategy
---

# Backtesting Strategy

> Strategy with Backtesting. strategy dev.

## Быстрый старт
Этот навык на русском языке. Оригинал: `backtesting-strategy`.

## Когда использовать
- Работа с Финансы
- Выполнение задач, связанных с Backtesting Strategy
- Профессиональное развитие

## Инструкции
1. Ознакомьтесь с описанием навыка
2. Изучите английскую версию для полных инструкций
3. Примените полученные знания на практике

## Ресурсы
- Оригинальный навык: `finance/backtesting-strategy/SKILL.md`
- Категория: Финансы
- Язык: Русский

## Валидация
- Прочитайте английскую версию для проверки
- Выполните описанные шаги
- Убедитесь в правильности результата
